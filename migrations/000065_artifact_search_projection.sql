-- +goose Up
-- A read-only, format-neutral projection for management search. The view
-- intentionally contains metadata only; artifact bytes remain in object
-- storage and authorization is still enforced by the application layer.
...
-- +goose Down
DROP VIEW IF EXISTS artifact_search_projection;

...
- legacy 读取的默认值现在显式化，不再静默。未配置任何 reader 模式的部署会放行未匹配的已认证调用方读取旧仓库——这是此前的文档化姿态；现在该情况会在启动时打印告警，且可用 `GATEWAY_LEGACY_READ_DEFAULT=deny` 立即拒绝这类调用方。下一版本会把 `deny` 变为默认值，因此本版先把升级动作写进旧 Group 迁移指南：盘点没有授权却读取的主体，为每个主体在 `GATEWAY_REPOSITORY_READERS` 中配置精确仓库名或 `prefix/*` 模式（或改用按仓库授权），然后设置该变量。该机制没有通配一切的模式，因此每个主体需要的仓库都必须逐一列出。管理员、全局只读角色、仓库自身的授权，以及待审批与待改密拦截均不受影响。

...
- 仓库授权现在可以按行写入。`POST /repositories/{repositoryId}/grants` 创建或替换由「主体 + 资源前缀」标识的单条授权，同一路径的 `DELETE` 只删除这一行，行已不存在时返回 `404`。两个操作都不需要 `If-Match` 前提条件，因为写入被限制在单行内，不会覆盖其他主体上的并发修改——这正是授权列表变长后整表 `PUT` 的风险所在。两个操作都会推进授权集版本号，并分别审计为 `repository.grants.upsert` 与 `repository.grants.delete`；整表替换保留给授权模板和批量编辑。

- 控制台的授权编辑器改为按行操作，不再在幕后整表替换。仓库的「访问授权」标签页以表格列出授权，每行带编辑与移除操作；它与「用户详情 → 仓库授权」面板都改走单条授权端点，其他主体上的并发修改不再会被悄悄覆盖，两个界面的 `412` 版本冲突重试逻辑也随之消失。把一条授权改到另一个资源前缀时，先 upsert 新行、再删除旧行，一次编辑不会把一条授权拆成两条。

- 仓库详情页的标签间距不再重复叠加：之前两个标签之间的空隙是 12px 的 gutter 加上两侧各 10px 的内边距，第一个标签还相对内容边缘内缩了 10px。现在间距只由 24px 的 gutter 决定，标签之间恰好相隔 24px，第一个标签与上方的摘要、下方的内容面严格对齐。

- 审计日志的「操作」列现在显示可读的中文/英文标签，而不是原始机器码：`repository.grants.upsert` 显示为「写入仓库授权」，流量类操作码显示为「读取（GET）」等，覆盖网关当前写入的全部操作码。原始代码保留在单元格的悬浮提示里，操作筛选器和 CSV 导出也仍使用原始代码；控制台不认识的代码（比如更新版本的后端写入的）会原样显示代码本身，而不是留白。

- 授权行的身份改由唯一的共享 helper 生成，不再有三套手写 key。仓库表格里一处用空格拼接主体与资源前缀、另一处用连字符，文件里还混进了一个字面 NUL 字节（git 与 grep 因此把该文件当二进制），不同的两条授权可能塌成同一个 React key，删一行会连带影响另一行。改主体或资源前缀时改为**先删除旧行、再写入新行**，删除失败时授权只会变窄或保持不变，而不会比管理员要求的更宽。

- 授权写入的规则收敛为共享的校验函数：单条端点、整表替换与授权模板编辑器三处一致。此前单条端点已经分叉——upsert 不限制主体长度、前缀放宽到格式路径上限，而 delete 拒绝超过 512 字节的主体、超过 255 的前缀以及控制字符，于是能建出一条两个控制台界面都删不掉的授权。现在写入严格按契约声明的形状校验，删除只要求能定位到已存的行，因此更早版本写入的行依然可删。控制台的主体与前缀输入框也带上了同样的上限。

- 审计页的「结果」列也改为可读文案。此前除两个取值外都显示后端代码，筛选器又是第三种「中英同串」的写法；现在 outcome 共用一张双语表，筛选器同时支持按可读标签与原始代码搜索，「失败请求」与「拒绝访问」也不再重复统计同一批拒绝。操作码映射补上了权限拒绝审计与 APT 包级操作实际写入的码（admin、intelligence、apt.package.*），并把仅为兼容更早版本数据而保留的标签单独标注。

- 控制台不再在一次失败请求后丢掉已加载的数据，也不再在首次加载失败时于错误横幅下继续转圈。任务中心与 Webhook 面板改为互斥地渲染「加载中 / 错误 / 空态 / 内容」四者之一；总览、分组、API 密钥页面在失败时保留原有数据、错误显示在其上方，而不是整页被一条横幅替换。剩下三处任务标签栏统一为与仓库详情相同的「单一 24px gutter」间距；两张漏掉共享基线类的表格补齐；原本只有一行灰字的空态改用统一空态；授权行操作的按钮与确认文案统一为「移除」。
## 0.4.0 - 2026-09-26

- 等级收敛的迁移补上了它欠下的那份报告——那些它自己无法决定的实例。`make member-role-migration-report` 只读取某个部署的数据库、不写任何东西，因此升级前可当清单、升级后可当确认。它输出四节，每行都给出要处理的标识：仍仅靠 `GATEWAY_REPOSITORY_READERS` / `GATEWAY_REPOSITORY_WRITERS` 访问仓库、且在其读取的仓库上没有授权的主体（含判定次数与最后一次时间）；由**无用户行**的 API key 与服务账号持有的授权（含凭据名称与状态）；模型无法表达的取值——等级不是 `none`/`member`/`admin` 的账号、含多个等级或未知等级的 key；以及不再生效的授权——仓库缺失或已非活跃，或名字为 `user:` / `api-key:` / `service-account:` 前缀但对象不存在。不带这些前缀的 token 主体**有意不列**。升级校验脚本会为每一类播入一个实例，并断言报告列出它、且不去认领一个已经持有授权的主体；它还会保留探查库升级前的 dump、把它还原，并断言还原后的库仍带着被移除的等级、而不是收敛后的约束——这条迁移的文档化回滚因此是被验证过的，而非假定成立。

- 没有任何仓库授权的 `member` 现在会被告知仓库权限从何而来。仓库目录的空态原本在讲"如何新建仓库"——这既不是该成员能做的事，也不是列表为空的原因；现在它说明真正的原因：仓库权限由平台管理员**按仓库**分配。平台管理员看到的仍是创建引导文案。

- 仓库详情页现在按**该仓库自身的权限**决定可见面，而不再看平台标记。管理类标签页——访问授权、保留策略、安全准入、容量、墓碑、晋升 / 复制、生命周期任务、签名快照、设置——对**该仓库的管理员**出现；发布需要写权限，制品扫描需要独立的 intelligence 权限，制品与使用统计对任何读权限开放。平台管理员保留全部标签页；无对应权限时即使 URL 直接指向该标签也不会呈现。

- V1 兼容面仍属平台管理员操作。`/api/v1/**` 处理器**不参与平台/仓库两档拆分**：它们早于按仓库权限模型，且 V1 Group 的成员可能完全没有仓库绑定，因此没有任何信息能把它归到某个管理员所持有的仓库上。两档模型落在 v2 面；需要按仓库管理的 V1 客户端应迁移到 `/api/v2`。

...
- 新增 `member`（成员）账号角色，其本身不附带任何仓库能力。把 SSO 账户批准为 `member`、或创建 `member` 的本地账户，都不会带来任何仓库访问权；其可访问范围完全来自按仓库授权，因此一次角色分配不再可能把全部仓库交给某个用户。该角色对用户、API 密钥、OIDC 角色映射与 OIDC JIT 默认角色均可用，在 Console 的角色选择器中排在首位并带明确说明，有效访问解释器也会标注它。既有 `reader` 与 `writer` 角色行为不变、仍覆盖全部仓库，因此没有部署会因此失去或获得访问权；它们为兼容保留，并由按仓库授权取代。

- 仓库配置变更与删除现在要求仓库 `admin` 作用域，而不再是 `write`。修改仓库的 endpoint、允许主机、出口代理、上游凭据、匿名读与严格发布设置，以及停用整个仓库，都会改变该仓库对所有客户端的行为，因此它们现在与授权、保留、安全准入、生命周期操作同属一档，而不再与"发布/删除制品"同档。仅持 `write` 作用域的主体——包括此前通过角色直通到达配置面的全局 writer——会被拒绝并返回 `403`。Console 中这些操作的界面此前已是管理员专属。
...
- Console 现在可供非管理员身份使用。全局 reader 或 writer 能看到受限的仓库目录（仅含其可读取的仓库），可打开仓库、浏览制品，并且只有在仓库授予写权限时才使用发布与上传入口；reader 保持只读。管理员专属页面——Group、访问控制、审计、身份提供方、API 密钥、服务账号、用户、站点设置与运维——仍仅管理员可见，路由守卫改为按整段路由前缀与尾斜杠匹配，不再使用精确路径字符串。仓库标签页按有效权限显示：设置、访问授权、保留策略、安全准入、容量、晋升、生命周期任务、墓碑与 APT 签名快照需要仓库管理权限，制品扫描需要 intelligence 作用域，发布需要写权限。标签页渲染前会等待有效权限，因此无权限的标签页不会闪现，深链接会回落到制品页。

- `GET /api/v2/repositories` 现在提供按权限过滤的仓库目录，不再要求管理员身份。管理员仍可见全部仓库；其他已认证主体只能看到其全局角色或仓库授权允许读取的仓库，待审批账号返回 `403 authorization_pending`。过滤在服务端执行，因此空结果不会泄露仓库是否存在。这为 Console 面向 reader/writer 角色的仓库目录扫清了服务端障碍。

...
func (h nativeMavenHandler) deploy(w http.ResponseWriter, r *http.Request) {
	...
	if name == "maven-metadata.xml" || strings.HasSuffix(name, ".sha1") || strings.HasSuffix(name, ".sha256") || strings.HasSuffix(name, ".md5") {
		http.Error(w, "Maven metadata and checksums are generated by the gateway", http.StatusBadRequest)
		return
	}

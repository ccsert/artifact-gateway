...
type MemoryStore struct {
	...
	consoleThemePackages   map[string]ConsoleThemePackage
	...
	authorizationRoles     map[string]AuthorizationRole
	retentionPolicies      map[string]RepositoryRetentionPolicy
	securityPolicies       map[string]RepositorySecurityPolicy
	quarantineReadPolicies map[string]RepositoryQuarantineReadPolicy
	...
	aptPublicationKeys     map[string]aptPublicationIdempotencyRecord
	aptPackageRevisions    map[string]APTPackageRevision
	aptPackageIdentities   map[string]string
	aptArchiveRestores     map[string]APTArchiveRestore
	aptArchiveObjects      map[string]map[string]APTArchiveObjectIntent
	aptLifecycleResults    map[string]APTLifecycleResult
	aptDeletions           map[string]APTPackageDeletion
	aptRetiredAt           map[string]time.Time
	aptPoolPaths           map[string]map[string]APTSnapshotAsset
	aptSnapshots           map[string]APTRepositorySnapshot
	aptSnapshotPackages    map[string][]APTSnapshotPackage
	aptSnapshotAssets      map[string][]APTSnapshotAsset
	aptSnapshotObjects     map[string]map[string]APTSnapshotObjectIntent
	...
	artifactQuarantines    map[string]ArtifactQuarantine
...
type aptPublicationIdempotencyRecord struct {
	payload, sessionID string
	expiresAt          time.Time
}

func NewMemoryStore() *MemoryStore {
	return &MemoryStore{groups: make(map[string]Group), mavenGroups: make(map[string]Group), rawGroups: make(map[string]Group), conanGroups: make(map[string]Group), hostedRepositories: make(map[string]HostedRepository), hostedGroups: make(map[string]HostedGroup), repositoryGrants: make(map[string]RepositoryGrantSet), authorizationTemplates: make(map[string]AuthorizationTemplate), authorizationRoles: make(map[string]AuthorizationRole), retentionPolicies: make(map[string]RepositoryRetentionPolicy), securityPolicies: make(map[string]RepositorySecurityPolicy), quarantineReadPolicies: make(map[string]RepositoryQuarantineReadPolicy), capacityQuotas: make(map[string]int64), idempotencyRecords: make(map[string]idempotencyRecord), mavenSessions: make(map[string]MavenPublishSession), mavenUploads: make(map[string]map[string]string), mavenAssets: make(map[string]MavenAsset), mavenArtifacts: make(map[string]MavenArtifact), mavenSessionKeys: make(map[string]idempotencyRecord), mavenCommitKeys: make(map[string]mavenCommitRecord), mavenObjectIntents: make(map[string]mavenObjectIntent), mavenObjectRefs: make(map[string]bool), conanObjects: make(map[string]ConanObjectIntent), conanSessions: make(map[string]ConanPublishSession), conanUploads: make(map[string]map[string]string), conanAssets: make(map[string]ConanAsset), conanRecipes: make(map[string]ConanRecipeRevision), conanPackages: make(map[string]ConanPackageRevision), ociUploads: make(map[string]OCIUpload), ociBlobs: make(map[string]OCIBlob), ociRepositoryBlobs: make(map[string]map[string]bool), ociManifests: make(map[string]OCIManifest), ociDeletedManifests: make(map[string]ociDeletedManifest), ociTags: make(map[string]string), ociUploadLocks: make(map[string]*sync.Mutex), ociObjectLocks: make(map[string]*sync.Mutex), rawAssets: make(map[string]RawAsset), rawObjects: make(map[string]RawObject), rawAssetTombstones: make(map[string]RawAsset), rawObjectLocks: make(map[string]*sync.Mutex), rawUploads: make(map[string]RawUpload), rawUploadLocks: make(map[string]*sync.Mutex), npmPackages: make(map[string]NPMPackage), npmVersions: make(map[string]NPMVersion), npmProxyLocks: make(map[string]*sync.Mutex), npmObjectLocks: make(map[string]*sync.Mutex), pypiFiles: make(map[string]PyPIFile), pypiObjectLocks: make(map[string]*sync.Mutex), goVersions: make(map[string]GoModuleVersion), goAssets: make(map[string]GoModuleAsset), goObjectLocks: make(map[string]*sync.Mutex), aptAssets: make(map[string]APTAsset), aptObjectLocks: make(map[string]*sync.Mutex), aptPublicationSessions: make(map[string]APTPublicationSession), aptPublicationKeys: make(map[string]aptPublicationIdempotencyRecord), aptPackageRevisions: make(map[string]APTPackageRevision), aptPackageIdentities: make(map[string]string), aptArchiveRestores: make(map[string]APTArchiveRestore), aptArchiveObjects: make(map[string]map[string]APTArchiveObjectIntent), aptLifecycleResults: make(map[string]APTLifecycleResult), aptDeletions: make(map[string]APTPackageDeletion), aptRetiredAt: make(map[string]time.Time), aptPoolPaths: make(map[string]map[string]APTSnapshotAsset), aptSnapshots: make(map[string]APTRepositorySnapshot), aptSnapshotPackages: make(map[string][]APTSnapshotPackage), aptSnapshotAssets: make(map[string][]APTSnapshotAsset), aptSnapshotObjects: make(map[string]map[string]APTSnapshotObjectIntent), ociObjectIntents: make(map[string]OCIObjectIntent), artifactTombstones: make(map[string]ArtifactTombstone), artifactIntelligence: make(map[string]ArtifactIntelligence), artifactQuarantines: make(map[string]ArtifactQuarantine), lifecycleJobs: make(map[string]LifecycleJob), auditCleanupJobs: make(map[string]AuditCleanupJob), replicationPlans: make(map[string]ReplicationPlan), replicationKeys: make(map[string]string), replicationChecks: make(map[string]map[string]ReplicationCheckpoint), apiKeys: make(map[string]APIKey), serviceAccounts: make(map[string]ServiceAccount), users: make(map[string]User), userIdentities: make(map[string]UserIdentity), userSessions: make(map[string]UserSession), runtimeNodes: make(map[string]RuntimeNode), scheduledTasks: make(map[string]ScheduledTask), scheduledTaskRuns: make(map[string]ScheduledTaskRun), webhookSubscriptions: make(map[string]WebhookSubscription), webhookEvents: make(map[string]WebhookEvent), webhookDeliveries: make(map[string]WebhookDelivery), consoleThemePackages: make(map[string]ConsoleThemePackage)}
}

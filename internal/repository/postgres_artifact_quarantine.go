...
func (s *PostgresStore) ReplaceArtifactQuarantine(ctx context.Context, value ArtifactQuarantine, expectedVersion string) (ArtifactQuarantine, error) {
	...
	if expectedVersion == "0" {
		...
		created, err := scanArtifactQuarantine(s.db.QueryRowContext(ctx, `INSERT INTO artifact_quarantines (repository_id,format,coordinate,digest,state,reason,updated_by,version,quarantined_at,released_at)
...
RETURNING `+artifactQuarantineColumns, value.RepositoryID, value.Format, value.Coordinate, value.Digest, value.State, value.Reason, value.UpdatedBy))
	...
	updated, err := scanArtifactQuarantine(s.db.QueryRowContext(ctx, `UPDATE artifact_quarantines
...
RETURNING `+artifactQuarantineColumns, value.RepositoryID, value.Format, value.Coordinate, value.Digest, expectedVersion, value.State, value.Reason, value.UpdatedBy))
	...
	var exists bool
	if queryErr := s.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM artifact_quarantines WHERE repository_id::text=$1 AND format=$2 AND coordinate=$3 AND digest=$4)`, value.RepositoryID, value.Format, value.Coordinate, value.Digest).Scan(&exists); queryErr != nil {

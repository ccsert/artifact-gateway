...
assert_json 'map(select(.kind == "Deployment" and .metadata.name == "artifact-gateway" and any(.spec.template.spec.containers[]; .name == "reference-apt-signer" and any(.env[]; .name == "REFERENCE_APT_SIGNER_KEY_FILE" and .value == "/var/lib/reference-apt-signer/keys/apt-release-private.gpg")))) | length == 1' 'APT signer key path in a process-owned subdirectory below the PVC mount'

...
import {
  ...
  ConanArtifactDetail,
  RawArtifactDetail,
} from "../artifact-detail/ArtifactRowDetail";
import { OciImageDetail } from "../artifact-detail/OciImageDetail";
...
export function SearchPage() {
  ...
  const columns: ColumnsType<SearchTableRow> = [
    ...
    },
    {
      ...
      width: 150,
      render: (_, tableRow) => (
        <ArtifactSecurityBadge
          summary={
            isMavenGroup(tableRow.row)
              ? mavenIntelligence(tableRow.row.hits)
              : tableRow.row.intelligence
          }
          text={text}
        />
      ),
    ...
    <div>
      ...
      {!q ? (
        <EmptyState
          ...
          hint={text(
            "支持 Maven 坐标、npm 包名、PyPI 项目、Go 模块、OCI 镜像、Conan 引用、Raw 路径和完整 SHA-256",
            "Supports Maven coordinates, npm packages, PyPI projects, Go modules, OCI images, Conan references, Raw paths, and full SHA-256 digests",
          )}
        />
      ) : error && hits.length === 0 ? (

      ...
      >
        ...
        <Table<DraftGrant>
          ...
          columns={[
            ...
            { title: text("权限", "Scopes"), key: "scopes", width: 200, render: (_, grant) => <Select className="w-full" value={grant.scopes[0]} options={[{ value: "repositories:read", label: text("读取", "Read") }, { value: "repositories:write", label: text("写入", "Write") }, { value: "repositories:admin", label: text("管理员", "Admin") }]} onChange={(value) => setDraftGrants((current) => current.map((item) => item.key === grant.key ? { ...item, scopes: [value] } : item))} /> },

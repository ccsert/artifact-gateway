...
- The legacy read default is now explicit instead of silent. A deployment that configures no reader patterns admits an unmatched authenticated caller to a legacy repository, which was the documented posture; it now logs a startup warning, and `GATEWAY_LEGACY_READ_DEFAULT=deny` refuses such callers immediately. The next release makes `deny` the default, so this release ships the upgrade action in the legacy group migration guide: inventory the actors that read without a grant, give each an exact repository name or `prefix/*` pattern in `GATEWAY_REPOSITORY_READERS` or a per-repository grant, then set the variable. There is no catch-all pattern, so every repository an actor needs has to be listed. Administrators, global read roles, a repository's own grants, and the pending and password-change blocks are unchanged.

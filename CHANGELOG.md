...
- The Console is now usable by non-administrator identities. A global reader or writer sees the repository catalog limited to repositories it can read, can open a repository, browse its artifacts, and use the publish and upload surfaces only when that repository grants write; a reader stays read-only. Administrator screens — groups, access control, audits, identity providers, API keys, service accounts, users, site settings, and operations — remain administrator-only, and the guard now matches whole route prefixes and trailing slashes instead of exact path strings. Repository tabs follow effective permission: settings, access grants, retention, security, capacity, promotion, jobs, tombstones, and APT snapshots require repository admin, scanning requires the intelligence scope, and publish requires write. Effective permissions are awaited before tabs render, so an unavailable tab never flashes and a deep link falls back to artifacts.

- `GET /api/v2/repositories` now serves a permission-scoped catalog instead of requiring an administrator. An administrator still sees every repository; any other authenticated principal sees only the repositories its global role or its repository grants allow it to read, and a Pending account is refused with `403 authorization_pending`. Because the filter runs on the server, an empty result never reveals whether a repository exists. This unblocks the Console repository catalog for reader and writer roles.

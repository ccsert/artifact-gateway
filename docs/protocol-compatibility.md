...
| Raw Hosted / Proxy / Group | Native Raw repository rooted at `/raw/<repository>/<path>`; authenticated PUT, GET, HEAD, single byte Range, Digest verification, ETag/Digest response headers, DELETE, content-addressed object storage, streaming GET/Range through `Open`/`OpenRange`, paginated directory-prefix listing through a trailing-slash GET/HEAD, derived `.sha256`/`.sha512` sidecars, and resumable uploads through `POST ?resumable=1`, immutable offset-addressed PATCH chunks, offset status, one-pass digest-verified completion, and cancellation. Proxy/Group use HTTPS allowlisted upstreams, bounded-buffer temporary staging, renewable distributed single-flight, stream-backed cache publication/read, and a configurable per-Gateway staging admission limit. A default-disabled Hosted quarantine-read policy rejects quarantined paths and checksum sidecars, hides them from listings, and prevents Group fallback. | Conditional write/update semantics, repository browsing beyond a prefix projection, and non-HTTP client tooling. Deployments must still provide a temporary volume, free-space monitoring, and hard ephemeral-storage limits. | Nexus Raw Hosted has a broader browsing/admin experience. Artifact Gateway exposes visible path metadata from PostgreSQL, validates sidecar assertions without persisting them, keeps resumable bytes invisible until digest-verified metadata promotion, and keeps bytes content-addressed with metadata-driven delete and delayed object reclamation. | `go test ./internal/app ./internal/repository ./contracts`, `make native-raw-e2e`, integration coverage for large-object Hosted/Proxy streaming, staging admission, HEAD and Range behavior, listing, derived checksums, resumable cross-instance completion, tombstones, orphan collection, Digest/ETag, and PostgreSQL/RustFS lifecycle. |

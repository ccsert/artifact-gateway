...
| Go Proxy / Group | Standard `GOPROXY` reads rooted at `/go/<repository>/<escaped-module>/...`; `@v/list`, `@latest`, `.info`, `.mod`, and `.zip`; module/version escaping; `go.mod` and module ZIP identity validation; SHA-256 content-addressed caching; ETag/HEAD; stale version lists and offline assets; anonymous and resource-prefix grant filtering; Group version aggregation; search, capacity, Console version selection, usage snippets, and version deep links. | Hosted publication, delete/restore, retention, promotion, replication, checksum database mirroring, and authenticated upstream proxies. | Nexus does not provide a first-class Go module repository format. Artifact Gateway implements the public Go Proxy protocol without inventing a non-standard upload workflow. | `go test ./internal/app ./internal/repository`, `make native-go-e2e`, and PostgreSQL/RustFS cross-instance cache, immutable identity, search, capacity, and offline-read coverage. |
| APT Proxy / Group | Verbatim reads rooted at `/apt/<repository>/...` for `dists/` metadata, signatures, indexes, by-hash objects, and `pool/` packages; SHA-256 object caching; ETag/HEAD; anonymous and resource-prefix grant filtering; ordered Group fallback; capacity, search, Console browse, and source snippets. The non-advertised Hosted preview has H1 staging plus an H2 atomic-snapshot core: deterministic `Packages`/gzip, Release checksums, by-hash assets, signer output, PostgreSQL visibility switching, and Hosted GET/HEAD/range reads are implemented and covered across PostgreSQL/RustFS. There is not yet a supported management trigger or deployable signer, so staged bytes and incomplete snapshots remain absent from protocol reads. | Installable Hosted publication, supported snapshot-publish management API, deployable production signer/key operations, Hosted capacity/search/Console projection, real APT install gate, delete/restore, retention, promotion, replication, upstream authentication, and automatic signing-key distribution. | Nexus supports APT through ecosystem-specific repository formats and plugins. Artifact Gateway keeps advertised APT support Proxy/Group-only until atomic signed Hosted snapshots and production signing pass their gates. | `go test ./internal/app ./internal/aptpublication ./internal/repository ./contracts`, `make native-apt-e2e`, and PostgreSQL/RustFS migration, staging, cleanup, signed-snapshot, capacity, and search coverage. |

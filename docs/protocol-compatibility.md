...
| npm Hosted / Proxy / Group | Standard npm packument and tarball reads under `/npm/<repository>/<package>`; npm CLI publication of immutable SemVer versions to Hosted; scoped packages; `latest` and custom dist-tags; anonymous install/audit under repository policy; package-level browse with exact version deep links. Proxy provides verified metadata and tarball read-through caching, conditional revalidation, negative caching, stale metadata on upstream failure, bounded retry and a distributed circuit breaker. Group merges Hosted and Proxy versions behind one Registry URL, keeps Hosted-first/member-order conflict semantics, rewrites tarball URLs to the Group, and filters members by grants and anonymous policy. Hosted versions support retention, tombstone/restore, delayed reclaim, promotion, and checkpointed replication. | Authenticated upstream registries, dist-tag mutation after publication, unpublish/deprecate, and vulnerability-database integration. | Nexus exposes broader npm security integration. Artifact Gateway emphasizes immutable verified caching and explicit egress policy; only Hosted advertises lifecycle mutation. | `go test ./internal/protocol/npm ./internal/app ./internal/repository`, `make native-npm-e2e`, PostgreSQL/MinIO cross-instance cache coverage, migration/capacity/search checks, and real npm CLI Hosted publication plus Group online/offline mixed-member install coverage. |
| PyPI Hosted / Proxy / Group | Standard twine multipart uploads at `/pypi/<repository>/legacy/`; PEP 503 HTML and PEP 691 JSON Simple APIs; wheel/sdist core-metadata identity checks; immutable SHA-256 distribution files; real pip installation; anonymous reads; project browse with searchable exact-version deep links. Proxy requires SHA-256 upstream links, verifies archive metadata before caching, and serves cached files offline. Group merges Hosted and Proxy files with Hosted-first conflicts. Hosted versions support retention, tombstone/restore, delayed reclaim, promotion, and checkpointed replication. | Authenticated upstream indexes, yanking, project metadata APIs beyond Simple, and vulnerability-database integration. | Nexus exposes additional PyPI administration. Artifact Gateway makes archive identity, upstream digests, and egress allowlists mandatory and uses version-scoped lifecycle operations. | `go test ./internal/app ./internal/repository`, `make native-pypi-e2e`, and PostgreSQL/MinIO cross-instance publication, search, tombstone, and restore coverage. |
| Go Proxy / Group | Standard `GOPROXY` reads rooted at `/go/<repository>/<escaped-module>/...`; `@v/list`, `@latest`, `.info`, `.mod`, and `.zip`; module/version escaping; `go.mod` and module ZIP identity validation; SHA-256 content-addressed caching; ETag/HEAD; stale version lists and offline assets; anonymous and resource-prefix grant filtering; Group version aggregation; search, capacity, Console version selection, usage snippets, and version deep links. | Hosted publication, delete/restore, retention, promotion, replication, checksum database mirroring, and authenticated upstream proxies. | Nexus does not provide a first-class Go module repository format. Artifact Gateway implements the public Go Proxy protocol without inventing a non-standard upload workflow. | `go test ./internal/app ./internal/repository`, `make native-go-e2e`, and PostgreSQL/MinIO cross-instance cache, immutable identity, search, capacity, and offline-read coverage. |
| APT Proxy / Group | Verbatim reads rooted at `/apt/<repository>/...` for `dists/` metadata, signatures, indexes, by-hash objects, and `pool/` packages; SHA-256 object caching; ETag/HEAD; anonymous and resource-prefix grant filtering; ordered Group fallback; capacity, search, Console browse, and source snippets. | Hosted publication, metadata generation/signing, delete/restore, retention, promotion, replication, upstream authentication, and automatic signing-key distribution. | Nexus supports APT through ecosystem-specific repository formats and plugins. Artifact Gateway keeps the first slice protocol-only so it never rewrites or invalidates upstream signed metadata. | `go test ./internal/app ./internal/repository ./contracts`, `make native-apt-e2e`, and PostgreSQL migration/capacity/search coverage. |
| Conan | Conan 2 Group/Proxy read-through and native Hosted resolution under `/conan/v2/<repository>/...`; Hosted publication sessions, recipe/package revision delete and restore, delayed reference-safe reclaim, visible-reference browse/search at `/api/v2/repositories/{id}/conan/references`, immutable promotion, and checkpointed replication. | Conan 1, general upstream index aggregation, remote-to-remote copying, and package administration beyond immutable revision lifecycle. | Nexus exposes broader Conan administration. Artifact Gateway keeps publication transactional, makes deletion reversible until reclaim, and separates authenticated management browse from the Conan download handshake. | `go test ./internal/app ./internal/repository`, Conan handler tests, PostgreSQL lifecycle/search/replication coverage, reclaim-worker integration evidence, and `make conan-e2e`. |
...
| Go | [Go Modules Reference: GOPROXY protocol](https://go.dev/ref/mod#goproxy-protocol) | Proxy and Group implement the standard read protocol, validate module assets with `golang.org/x/mod`, and remain explicitly read-only; `make native-go-e2e` runs a real `go mod download` online and again with a fresh client cache after the upstream stops. |

...
| Conan | Conan 2 Group/Proxy read-through under `/conan/v2/<group>/...`; authentication policy, proxy host allowlist, cache metadata, negative cache, path validation, and an optional managed `format: conan` Repository binding for runtime grants. | Native Conan Hosted write/delete/search, Conan package administration, replication, and native Conan artifact storage. | Nexus supports richer Conan repository management. Artifact Gateway keeps Conan data-plane behavior read-through only, while its managed Repository is an authorization target for a bound remote. | `go test ./internal/app`, Conan handler tests, and PostgreSQL coverage for bound member persistence. |

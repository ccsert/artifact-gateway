...
| npm Hosted | Standard npm packument and tarball reads under `/npm/<repository>/<package>`; npm CLI publication of immutable SemVer versions; scoped packages; `latest` and custom dist-tags; SHA-512 integrity, SHA-1 shasum, SHA-256 metadata; anonymous install/audit under repository policy; package-level browse with exact version deep links. | npm Proxy and Group repositories, dist-tag mutation after publication, unpublish/deprecate, retention, restore, promotion, replication, and vulnerability-database integration. | Nexus exposes npm Proxy/Group and broader lifecycle administration. Artifact Gateway starts with a deliberately narrow Hosted slice and advertises only `read`, `publish`, and `browse`. | `go test ./internal/protocol/npm ./internal/app ./internal/repository`, `make native-npm-e2e`, PostgreSQL migration/capacity/search checks, and real npm CLI publish/install coverage. |
...
| npm | [npm registry API package metadata](https://github.com/npm/registry/blob/main/docs/REGISTRY-API.md) | The Hosted handler accepts npm CLI publication documents and serves authoritative packuments/tarballs; `make native-npm-e2e` is the executable compatibility overlay. |

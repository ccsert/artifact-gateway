...
| APT Proxy / Group | Verbatim reads rooted at `/apt/<repository>/...` for `dists/` metadata, signatures, indexes, by-hash objects, and `pool/` packages; SHA-256 object caching; ETag/HEAD; anonymous and resource-prefix grant filtering; ordered Group fallback; capacity, search, Console browse, and source snippets. The non-advertised Hosted H2 preview supports idempotent management staging and snapshot publication, deterministic `Packages`/gzip, Release checksums, by-hash assets, a loopback reference signer, PostgreSQL visibility switching, Hosted GET/HEAD/range reads, capacity/search/Console projection, and a real signed Debian update/install gate. Staged bytes and incomplete snapshots remain absent from protocol reads. | Production signer/key custody and rotation, operator signing state, automatic key distribution, delete/restore, retention, promotion, replication, and upstream authentication. | Nexus supports APT through ecosystem-specific repository formats and plugins. Artifact Gateway keeps advertised APT support Proxy/Group-only until H3 production signing passes its gates. | `go test ./internal/app ./internal/aptpublication ./internal/repository ./contracts`, `make native-apt-e2e`, and PostgreSQL/RustFS migration, staging, cleanup, signed-snapshot, capacity, and search coverage. |

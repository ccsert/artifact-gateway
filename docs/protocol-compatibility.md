...
| Raw Hosted / Proxy / Group | Native Raw is rooted at `/raw/<repository>/<path>` and supports authenticated PUT, GET, HEAD, one byte Range, Digest/ETag, DELETE, streaming object reads, Hosted trailing-slash prefix listing, derived `.sha256`/`.sha512` sidecars, and resumable upload with immutable chunks and one-pass completion. Proxy/Group use HTTPS-allowlisted upstreams, bounded temporary staging, renewable distributed single-flight, streaming cache publication/read, and per-Gateway staging admission. A default-disabled Hosted quarantine-read policy hides paths, sidecars, and Hosted listing entries. | Conditional write/update semantics, multipart Range, browsing beyond the Hosted prefix projection, and non-HTTP tooling. Deployments still need a temporary volume, free-space monitoring, and hard ephemeral-storage limits. | Nexus Raw Hosted has a broader browse/admin experience. Gateway keeps incomplete bytes invisible, validates sidecar assertions without storing duplicate assets, and uses metadata-driven deletion plus delayed object reclamation. | `go test ./internal/app ./internal/repository ./contracts`, `make native-raw-e2e`, and integration coverage for large-object streaming, staging admission, HEAD/Range, listing, checksums, resumable completion, tombstones, orphan collection, and PostgreSQL/RustFS lifecycle. |

...
Conan, and npm. Its unsupported rows describe the current release, not the long-term
...
| npm Hosted / Proxy | Standard npm packument and tarball reads under `/npm/<repository>/<package>`; npm CLI publication of immutable SemVer versions to Hosted; scoped packages; `latest` and custom dist-tags; anonymous install/audit under repository policy; package-level browse with exact version deep links. Proxy provides metadata and tarball read-through caching, conditional metadata revalidation, negative caching, stale metadata on upstream failure, bounded retry and a distributed circuit breaker; it enforces HTTPS/host allowlists, SHA-512 integrity, SHA-1 shasum, SHA-256 digest, and tarball package identity before committing cached bytes. | npm Group repositories, authenticated upstream registries, dist-tag mutation after publication, unpublish/deprecate, retention, restore, promotion, replication, and vulnerability-database integration. | Nexus exposes npm Group and broader lifecycle administration. Artifact Gateway's Proxy slice favors immutable verified caching and explicit egress policy; only Hosted advertises `publish`, while Hosted and Proxy advertise `read` and `browse`. | `go test ./internal/protocol/npm ./internal/app ./internal/repository`, `make native-npm-e2e`, PostgreSQL/MinIO cross-instance cache coverage, migration/capacity/search checks, and real npm CLI Hosted publish plus Proxy online/offline install coverage. |
...
| npm | [npm registry API package metadata](https://github.com/npm/registry/blob/main/docs/REGISTRY-API.md) | Hosted accepts npm CLI publication documents and serves authoritative packuments/tarballs. Proxy rewrites upstream tarball URLs, verifies immutable content before caching, and can serve installed versions while upstream is offline; `make native-npm-e2e` is the executable compatibility overlay. |

...
| APT Proxy / Group | Verbatim reads rooted at `/apt/<repository>/...` for `dists/` metadata, signatures, indexes, by-hash objects, and `pool/` packages; SHA-256 object caching; ETag/HEAD; anonymous and resource-prefix grant filtering; ordered Group fallback; capacity, search, Console browse, and source snippets. The non-advertised H1 management boundary explicitly provisions an APT Hosted preview repository, then reserves and stages one verified `.deb` for future signed-snapshot publication, with idempotency, quota, transactional audit, and durable orphan recovery; staged bytes are never protocol-visible. | Installable Hosted publication, `Packages`/`Release` generation and signing, signed-snapshot visibility, delete/restore, retention, promotion, replication, upstream authentication, and automatic signing-key distribution. | Nexus supports APT through ecosystem-specific repository formats and plugins. Artifact Gateway keeps advertised APT support Proxy/Group-only until atomic signed Hosted snapshots and production signing pass their gates. | `go test ./internal/app ./internal/aptpublication ./internal/repository ./contracts`, `make native-apt-e2e`, and PostgreSQL/RustFS migration, staging, cleanup, capacity, and search coverage. |

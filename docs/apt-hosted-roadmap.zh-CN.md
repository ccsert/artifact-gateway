...
已提供[签名快照归档导出与离线完整性校验](apt-snapshot-archive.zh-CN.md)。可信同仓库归档恢复已支持独立备份摘要与公钥校验。仍待完成：托管 KMS/HSM 私钥保管与恢复、部署告警。验收要求轮换前中后 Debian 验证成功，恢复仓库重现相同签名摘要；完成这些生产保管项前不得宣称 H3 完成。

## APT-H4：生命周期、扫描与分发

专用[删除、保留、恢复与快照清理](apt-hosted-lifecycle.zh-CN.md)已提供操作员 API；通用定时策略、Console 表单、隔离与分发仍需后续验收。

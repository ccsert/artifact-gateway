...
已提供[签名快照归档导出与离线完整性校验](apt-snapshot-archive.zh-CN.md)。可信同仓库归档恢复已支持独立备份摘要与公钥校验。仍待完成：托管 KMS/HSM 私钥保管与恢复、专用快照保留/重建、部署告警。验收要求轮换前中后 Debian 验证成功，恢复仓库重现相同签名摘要；完成这些生产保管项前不得宣称 H3 完成。

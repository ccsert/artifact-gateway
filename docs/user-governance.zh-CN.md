...
删除永久而非可恢复墓碑；账户只有一个角色，取值为待授权、成员或平台管理员。`member` 身份不附带任何仓库能力，只能访问被显式授权的仓库。旧有的 `reader`/`writer` 角色仍覆盖全部仓库，为兼容保留并正在被按仓库授权取代；尚无由任意权限组合的自定义角色与多角色同时分配；未实现密码过期或可配置复杂度；不支持 OIDC back-channel 或 IdP 发起登出。

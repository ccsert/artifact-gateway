...
在源 Hosted APT 仓库进入「签名快照」，输入源 suite，再选择「晋升 / 复制」。从该套件当前可见的软件包中选择源制品，指定另一个活跃的 Hosted APT 目标仓库及明确的目标 suite。源身份使用服务端返回的规范 `poolPath` 和 SHA-256；高级手动输入仍受相同的服务端准入检查约束。调用者需要两个仓库的管理权限，API 与 Worker 均需要签名配置。

晋升进度在目标仓库的「生命周期任务」中查看；复制进度位于计划列表，并展示持久化的目标 suite。提交失败或结果不确定时，在当前表单重试相同操作会沿用原幂等键。Console 明确标注操作员预览，不改变公开格式能力或 Hosted 仓库创建选项。

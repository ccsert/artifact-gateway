...
1. Go 兼容缺口已在 不削弱 Hosted 保证的前提下全部闭合。Go Proxy 的认证上游凭据已完成（`none`/`basic`/`bearer` 凭据以 `GATEWAY_SETTINGS_ENCRYPTION_KEY` 封装，只应用于 Go Proxy 上游抓取，跨主机重定向时剥离，在 Go Proxy 之外以明确的 `400` 拒绝），默认关闭的隔离读取强制已完成，覆盖 Go Hosted 与 Go Group；Go Proxy 现在可镜像一个已列入允许列表的校验和数据库——go 命令的 `/go/<repository>/sumdb/<name>/supported` 探测与 `/latest`、`/lookup/`、`/tile/` 请求经允许列表内的 HTTPS egress 原样转发签名字节、不做缓存，并由 `make native-go-e2e` 中真实 go 客户端的签名验证门禁证明。

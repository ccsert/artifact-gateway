...
APT Hosted 操作员预览提供专用的包删除、恢复、保留预览与快照清理 API。每次删除、恢复或保留操作都重新生成完整 Packages、Release 和两份签名；只有对象持久化、签名验证及数据库事务全部成功，才替换当前快照。当前公开格式能力仍为 Proxy/Group，生产 KMS/HSM、隔离与分发不在此阶段内。
...
- 当前 Release/Packages 只读 visible 快照。不可变 pool 与 by-hash 路径在 visible 或 retired 快照仍有引用时继续可读，使已缓存旧 Release 的客户端完成下载。这不是安全隔离机制；隔离策略另属后续阶段。
...
本阶段使用显式管理 API 执行保留和清理，尚未把 APT 接入通用 retention policy 定时执行器、Console 生命周期表单或隔离/晋级/复制流程。

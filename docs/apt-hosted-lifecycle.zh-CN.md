...
Console 在 Hosted APT 仓库的**签名快照**页签下提供这些显式操作：仓库管理员输入精确套件名，查看当前签名指纹与快照历史，选择软件包预览删除、预览保留清理，或恢复仍在恢复窗口内的删除记录。审阅面板列出受影响软件包与固定的基线快照；保留执行提交预览产生的候选 ID。冲突需要刷新并重新预览；执行结果不确定时可在审阅仍打开的情况下复用同一幂等键；刷新失败会保留最后一次证据并禁用操作，直到刷新成功。

快照历史只允许清理已退休且宽限期已过的快照，并需要显式确认；这不表示后台物理字节回收已完成。通用定时保留策略执行器对 APT 仍不可用。Console 不提供首次发布表单。

Console 在同一签名快照视图下另有**灾备归档**页签：把任一 visible 或 retired 快照导出为可携带归档，并同时给出 `sha256:` 回执供下载；恢复时只接受管理员在备份时独立保存的那一份回执，且拒绝从本次上传现算摘要。归档契约与离线 `apt-snapshot verify` 命令见[签名快照归档](apt-snapshot-archive.zh-CN.md)。

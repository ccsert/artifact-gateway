...
评估快照：2026-08-29。本评估描述项目处于 0.1.0 早期发布边界时的工程质量，不代表
生产认证或支持承诺。
...
| 文档 | 完整双语源集 | 每份站点文档都有英文规范路由和实质中文 companion；受测 site map 统一管理导航与覆盖。 |
| 可运维性 | 早期发布基线良好 | 健康检查、指标、诊断、恢复、Kubernetes 和分布式角色文档已具备；生产环境证据仍需独立取得。 |
| 性能 | 本地基线可复现 | 已测量二进制/镜像体积、静默内存、经鉴权的 PostgreSQL/RustFS 读取和一组 64 MiB 工程负载；受控类生产负载和 soak 尚未完成。 |
| 公开项目准备 | 公开早期版本 | 仓库已公开、具备版本化分发并采用 MIT License；公开安全报告渠道和明确支持承诺仍待补齐。 |
...
所有站点文档都具备英文规范路由与实质中文 companion，包括 ADR、契约、运维、发布证据
和研究材料。`docs/site-map.json` 提供框架中立导航；`make docs-check` 会拒绝缺失文档对、
单向语言链接、中文占位页、重复路由和不完整覆盖。

剩余风险已经从“首次覆盖”变为“持续同步”。行为、命令、兼容限制和预览状态发生变化时，
必须在同一评审中同步两种语言。

### 4. 发布证据范围大于默认测试目标

`make test` 保护共享本地行为，而完整协议 E2E、集成、浏览器、性能、轮换、升级和恢复
门禁属于独立目标。后续发布清单必须指向精确 CI 证据，不能把单个绿色命令当成完整发布
...
8. **维护发布矩阵。** 记录精确提交、干净工作区、不可变镜像、CI、
   集成、恢复、目标环境和具名批准证据。

...
38%。Console 对全部手写代码执行 50% 行、48.5% 语句、45% 函数、43.5% 分支门槛，并为重点
模块设置更高阈值。这些门槛能防止回退，但随着大型模块拆分和公共边界测试成本下降，
应该持续提高。

#### Vitest 4 统计口径迁移

Vitest 与 V8 覆盖率 provider 从 3.2.7 一同升级到 4.1.11，修复
[GHSA-82fw-gwwq-j7x9](https://github.com/advisories/GHSA-82fw-gwwq-j7x9)，没有增加审计例外。
受影响的是开发工具，不是 Gateway 运行时。Vitest 4 强制使用
[基于 AST 的 V8 映射](https://v4.vitest.dev/guide/migration#v8-code-coverage-major-changes)；
旧映射器统计源码行，并遗漏部分可执行分支和函数，两套百分比不能直接比较。

对 `669b230d` 的同一份 Console 源码和 239 个测试，分别使用原始锁文件、v3 实验性
AST 映射和 v4 测量；三轮之间没有修改生产源码、测试、覆盖范围或排除规则，239 个测试
均通过。覆盖项数/总项数如下：

| 映射器 | 行 | 语句 | 函数 | 分支 |
| --- | --- | --- | --- | --- |
| v3 默认 | 17599/30846 (57.05%) | 17599/30846 (57.05%) | 497/873 (56.93%) | 2336/3271 (71.41%) |
| v3 实验性 AST | 3214/6381 (50.36%) | 3378/6912 (48.87%) | 905/1996 (45.34%) | 2729/6218 (43.88%) |
| v4 AST | 3201/6381 (50.16%) | 3362/6912 (48.64%) | 905/1996 (45.34%) | 2716/6218 (43.67%) |

两轮 AST 测量的可执行项总数一致，均无法满足原先 53% 函数和 65% 分支门槛。新全局门槛
相对 v4 实测值的余量均小于一个百分点。两个 AST 版本的少量覆盖项数差异如实保留在表中，
没有把它们视为完全相同的结果。统计范围仍包含全部手写 Console 代码，所有重点模块门槛
保持不变，包括 Layout 的 90% 行/语句要求；新增测试覆盖 Token 取消/清除、受保护页面
登录跳转、身份加载、存储不可用和移动导航。后续更换统计口径同样需要同源码对照证据；
一般覆盖率回退必须补充有意义的测试，不能降低门槛或增加排除项。
